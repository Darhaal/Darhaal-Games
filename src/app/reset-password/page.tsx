...
function ResetPasswordContent() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4 font-sans text-[#1A1F26] relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#9e1316]/5 rounded-full blur-[100px] pointer-events-none"></div>

      <div className="w-full max-w-md bg-white border border-[#E6E1DC] p-8 rounded-[24px] shadow-xl shadow-[#9e1316]/5 relative z-10">

        <div className="text-center mb-8">
          <div className="flex justify-center mb-6">
             <div className="relative w-24 h-24 bg-[#F5F5F0] rounded-2xl flex items-center justify-center border border-[#E6E1DC] shadow-inner p-4 group">
                {/* Логотип */}
                <img
                  src="/logo512.png"
                  alt="Logo"
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.parentElement!.innerHTML = '<span class="text-2xl font-bold text-[#9e1316]">DG</span>';
                  }}
                />
             </div>
          </div>
          ...
          <p className="text-sm text-[#8A9099] font-medium">
        ...
        {status === 'success' ? (
          <div className="flex flex-col items-center justify-center py-6 animate-in fade-in zoom-in duration-300">
            <CheckCircle className="w-16 h-16 text-[#9e1316] mb-4 drop-shadow-md" />
            <p className="text-lg font-bold text-[#1A1F26] text-center">{msg}</p>
            <p className="text-sm text-[#8A9099] mt-2 font-medium">Redirecting...</p>
          </div>
        ) : (
          <form onSubmit={handleUpdatePassword} className="space-y-6">
             {status === 'error' && (
              <div className="flex items-center gap-3 bg-[#9e1316]/5 border border-[#9e1316]/20 text-[#9e1316] p-4 rounded-xl text-sm font-bold">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#8A9099] uppercase tracking-wider ml-1">
                Enter new password
              </label>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#9e1316] hover:bg-[#7a0f11] text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-[#9e1316]/20 active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2 text-xs uppercase tracking-widest hover:shadow-[#9e1316]/30"
            >
      ...
      <div className="mt-8 flex items-center gap-3 opacity-60 relative z-10">
