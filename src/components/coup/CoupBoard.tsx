...
export default function CoupBoard() {
  ...
  const { gameState, roomMeta, loading, performAction, startGame, leaveGame, pass, challenge, block, resolveLoss } = useCoupGame(lobbyId, userId);
  ...
  const isLosing = phase === 'losing_influence' && gameState.pendingLoserId === userId;
      ...
      <main className="flex-1 relative z-10 p-4 pb-60 flex flex-col max-w-6xl mx-auto w-full h-full overflow-y-auto custom-scrollbar">
        <div className="flex flex-wrap justify-center gap-4 pt-4">
          {players.map(p => {
            ...
            return (
              <div key={p.id} onClick={() => targetMode && performAction(targetMode, p.id)} className={`relative flex flex-col items-center p-3 bg-white border rounded-2xl transition-all ${isCurr ? 'ring-4 ring-[#9e1316] scale-105 z-20' : 'opacity-90'} ${targetMode ? 'cursor-pointer animate-pulse ring-4 ring-blue-400' : ''} ${p.isDead ? 'grayscale opacity-50' : ''}`}>
        ...
        {/* Reaction Bar */}
        {!isMyTurn && phase !== 'choosing_action' && phase !== 'losing_influence' && !me?.isDead && (
        ...
        {me && (
          <div className="fixed bottom-0 left-0 right-0 p-2 sm:p-4 z-50">
            <div className="max-w-4xl mx-auto bg-white/95 backdrop-blur-xl border border-[#E6E1DC] rounded-[32px] p-4 sm:p-6 shadow-2xl relative">
              {isMyTurn && !isLosing && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#9e1316] text-white px-4 py-1.5 rounded-full text-xs font-black uppercase shadow-lg animate-bounce z-20">{t.yourTurn}</div>}
              {isLosing && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-red-600 text-white px-6 py-2 rounded-full text-xs font-black uppercase shadow-lg animate-pulse z-30">CHOOSE CARD TO LOSE</div>}
